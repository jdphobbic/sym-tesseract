...
function renderEvents() {
    ...
    filtered.forEach(evt => {
        const prizesHtml = evt.prizes.map(p => `<span class="prize-badge">${p.rank}: ${p.amount}</span>`).join(' ');
...
function renderGalleryModal() {
    ...
    const prizesHtml = evt.prizes.map(p => `<span class="prize-badge" style="font-size: 0.85rem; padding: 4px 10px;">${p.rank}: ${p.amount}</span>`).join(' ');
